-- NOTE:
-- This migration name is retained for history continuity only.
-- Behavior is intentionally constrained to metadata patching on rows
-- that are ALREADY published. It does not insert rows and does not
-- change status counts.

begin;

create temp table _published_card_patch (
...
insert into _published_card_patch (
  listing_id, title_en, title_ja, source_url, ward,
  walk_min, train_min, total_transit_min, price_m, layout, sqm,
...
set
  ...
  ward = c.ward,
  walk_min = c.walk_min,
  train_min = c.train_min,
  total_transit_min = c.total_transit_min,
  price_m = c.price_m,
  layout = c.layout,
  sqm = c.sqm,
  station_name = c.station_name,
  station_line = c.station_line,
  display_date = c.display_date,
  updated_at = now()
from _published_card_patch c
where p.status = 'published'
  and (
    p.source_listing_id = c.listing_id
    or p.external_id like ('%' || c.listing_id)
    or p.source_url like ('%' || c.listing_id || '%')
  );
